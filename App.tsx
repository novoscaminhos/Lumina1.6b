...
import { LENORMAND_CARDS, LENORMAND_HOUSES, AFRODITE_HOUSES, FUNDAMENTALS_DATA, STUDY_BALLOONS } from './constants';
...
const App: React.FC = () => {
  ...
  const handleResetZoom = useCallback(() => {
    if (boardRef.current && contentRef.current) {
      ...
      const padding = spreadType === 'relogio' ? 40 : spreadType === 'templo-afrodite' ? 40 : 100; 
      ...
      // AJUSTE DINÂMICO PARA RELÓGIO E AFRODITE
      if (spreadType === 'relogio') {
        ...
        fitScale = Math.min(fitScale, 0.95);
      } else {
  ...
  const handleSaveReading = () => {
    ...
    const newReading = {
      id: Date.now(),
      title: `Leitura ${spreadType === 'mesa-real' ? 'Mesa Real' : spreadType === 'relogio' ? 'Relógio' : spreadType === 'templo-afrodite' ? 'Templo de Afrodite' : 'Mesa de 9'}`,
  ...
  const handleShuffle = () => {
    ...
    const newBoard = generateShuffledArray(36); // Generate full shuffle then slice in render or use specific array
    
    // Create board based on size, fill rest with null
    ...
    <div className={`min-h-screen flex flex-col md:flex-row bg-slate-50 text-slate-900 transition-colors overflow-hidden font-inter`}>
      ...
      <aside className={`fixed md:sticky top-0 inset-y-0 left-0 flex flex-col border-r border-slate-200 bg-white shadow-xl transition-all duration-300 z-[60] h-screen ${sidebarCollapsed ? 'w-16' : 'w-64'}`}>
        ...
        <nav className="px-2 space-y-2 overflow-y-auto custom-scrollbar shrink-0">
          ...
          <NavItem icon={<Heart size={18}/>} label="Afrodite" active={view === 'board' && spreadType === 'templo-afrodite' && !studyMode.active} collapsed={sidebarCollapsed} onClick={() => {setView('board'); setSpreadType('templo-afrodite'); setIsManualMode(false); setStudyMode(prev => ({ ...prev, active: false }));}} disabled={accessType === 'none'} />
      ...
      <main className="flex-grow flex flex-col h-screen overflow-y-auto custom-scrollbar relative">
        ...
        <header className={`h-16 flex items-center justify-between px-10 border-b sticky top-0 z-20 backdrop-blur-md transition-colors bg-white/95 border-slate-200 shadow-sm`}>
          <h2 className={`font-cinzel text-sm font-black tracking-widest uppercase text-slate-900`}>
            {view === 'home' ? 'Bem-vindo ao Lumina' : view === 'board' ? (studyMode.active ? 'Estudo Prático' : isHistoryView ? 'Visualizando Histórico' : isManualMode ? 'Mesa Personalizada' : spreadType === 'mesa-real' ? 'Mesa Real' : spreadType === 'templo-afrodite' ? 'Templo de Afrodite' : spreadType === 'mesa-9' ? 'Quadrado de 9' : 'Relógio') : view === 'glossary' ? 'Glossário' : view === 'fundamentals' ? 'Fundamentos' : view === 'profile' ? 'Perfil do Usuário' : view === 'study' ? 'Modo Estudo' : 'Estudo'}
          </h2>
          {/* Trial Timer */}
               ...
               {!studyMode.active && (
                 <div className={`flex p-1 rounded-xl border bg-white border-slate-200 shadow-sm border shadow-sm`}>
                   {(['nenhuma', 'ponte', 'cavalo', 'moldura', 'veredito', 'diagonais', 'centro', 'cruz', 'todas'] as any[]).map(f => {
                     ...
                     if ((spreadType === 'relogio' || spreadType === 'templo-afrodite') && !['nenhuma', 'todas'].includes(f)) return null;
        ...
        <div className="p-4 md:p-10 flex-grow flex flex-col min-h-0 relative">
          ...
          {view === 'home' && (
            <div className="max-w-6xl mx-auto w-full flex flex-col items-center justify-center py-10 animate-in fade-in duration-700">
               ...
               {/* Cards Home (Mantidos) */}
               <div className={`grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 w-full max-w-6xl transition-all duration-500 ${accessType === 'none' ? 'opacity-50 grayscale pointer-events-none filter blur-sm' : ''}`}>
                  ...
                  <div onClick={() => { setView('board'); setSpreadType('templo-afrodite'); setIsManualMode(false); }} className="group relative bg-white border border-rose-200 rounded-[2rem] p-8 hover:border-rose-500 transition-all cursor-pointer shadow-lg hover:shadow-2xl hover:shadow-rose-500/20 hover:-translate-y-1 overflow-hidden">
                    ...
                    <div className="relative z-10 flex flex-col h-full"><div className="w-14 h-14 bg-rose-100 rounded-2xl flex items-center justify-center text-rose-600 mb-6 group-hover:scale-110 transition-transform"><Heart size={28} /></div><h3 className="text-xl font-cinzel font-bold text-slate-900 mb-2">Afrodite</h3><p className="text-xs text-slate-500 leading-relaxed mb-6">7 Cartas. Análise profunda de relacionamentos e conexões afetivas.</p><div className="mt-auto flex items-center text-rose-600 text-xs font-black uppercase tracking-widest group-hover:gap-2 transition-all">Iniciar <ArrowRightLeft size={14} className="ml-2" /></div></div>
                  </div>
              ...
              <div ref={boardRef} className="flex-grow flex flex-col items-center justify-start min-h-0 w-full py-10 overflow-y-auto overflow-x-hidden custom-scrollbar scroll-smooth">
                <div className="flex flex-col items-center w-full transition-all duration-500" style={{ minHeight: `${unscaledHeight * zoomLevel}px` }}>
                  {spreadType === 'mesa-real' ? (
                    ...
                    <div ref={contentRef} className="grid grid-cols-3 gap-12 w-full max-w-3xl mx-auto items-center py-20 animate-in fade-in slide-in-from-top-10 duration-700" style={{ transform: `scale(${zoomLevel})`, transformOrigin: 'top center' }}>
                      {/* Coluna 1: Consultante */}
                      <div className="space-y-8 flex flex-col items-center">
                        <h4 className="font-cinzel text-xs font-black uppercase text-indigo-600 tracking-[0.2em] bg-indigo-50 px-4 py-2 rounded-xl border border-indigo-100 mb-4">Consultante</h4>
                        {[0, 1, 2].map(idx => (
                          <div key={idx} className="w-32"><CardVisual card={board[idx] ? LENORMAND_CARDS.find(c => c.id === board[idx]) : null} houseId={idx+1} isSelected={selectedHouse === idx} isThemeCard={false} onClick={() => handleHouseSelection(idx)} isAnimating={isAnimating} spreadType="templo-afrodite" /></div>
                        ))}
                      </div>
                      
                      {/* Coluna 2: A Conexão / Síntese */}
                      <div className="flex flex-col items-center space-y-8">
                        <div className="h-40" />
                        <h4 className="font-cinzel text-xs font-black uppercase text-rose-600 tracking-[0.2em] bg-rose-50 px-4 py-2 rounded-xl border border-rose-100 mb-4">Conexão</h4>
                        <div className="w-36 scale-110 shadow-2xl">
                           <CardVisual card={board[6] ? LENORMAND_CARDS.find(c => c.id === board[6]) : null} houseId={7} isSelected={selectedHouse === 6} isThemeCard={true} themeColor="rgba(225, 29, 72, 0.4)" onClick={() => handleHouseSelection(6)} isAnimating={isAnimating} spreadType="templo-afrodite" />
                        </div>
                        <div className="h-40" />
                      </div>
                      ...
                      {/* Coluna 3: Parceiro */}
                      <div className="space-y-8 flex flex-col items-center">
                        <h4 className="font-cinzel text-xs font-black uppercase text-slate-600 tracking-[0.2em] bg-slate-100 px-4 py-2 rounded-xl border border-slate-200 mb-4">Parceiro(a)</h4>
                        {[3, 4, 5].map(idx => (
                          <div key={idx} className="w-32"><CardVisual card={board[idx] ? LENORMAND_CARDS.find(c => c.id === board[idx]) : null} houseId={idx+1} isSelected={selectedHouse === idx} isThemeCard={false} onClick={() => handleHouseSelection(idx)} isAnimating={isAnimating} spreadType="templo-afrodite" /></div>
                        ))}
                      </div>
